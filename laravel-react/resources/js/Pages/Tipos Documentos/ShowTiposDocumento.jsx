...
const ShowTiposDocumento = ({auth}) => {
    //PERMISOS
    const {hasRoles,hasPermission} = usePermission()

    //VARIABLES QUE ENTREGA EL CONTROLADOR
    const { all_tipos_documento } = usePage().props;
    const [tipos_documento,setTiposDocumento] = useState(all_tipos_documento)
    ...
    const filteredItems = useMemo(() => {
        let filteredTiposDocumento = [...tipos_documento];
        if (hasSearchFilterNombre) {
            filteredTiposDocumento = filteredTiposDocumento.filter((direccion) => direccion.nombre.toLowerCase().includes(filterNombre.toLowerCase()));
        }
        return filteredTiposDocumento;
    }, [tipos_documento,filterNombre]);
    ...
    return (
        ...
        header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Tipos de Documento</h2>}>
            <Head title='Tipos de Documento' />
            <TitleTemplate>Tipos de Documento</TitleTemplate>
            <FilterTemplate>
                    ...
                    <div className="md:flex justify-center gap-4 items-end">
                        ...
                        <div className="flex justify-between items-center w-full gap-1 md:gap-3">
                            <div className='flex items-center'>
                                <span className="text-default-400 text-tiny lg:text-small">Total {tipos_documento.length} tipos de documento</span>
                            </div>
            ...
            <ContentTemplate>
                    ...
                    <div className='flex gap-3'>
                        {
                            ...
                            <>
                            <Link href={route('tipo-documento.create')}>
                ...
                <div className='w-full'>
                    <Table aria-label="Tabla tipos documentos" color={"primary"}
                    ...
                    classNames={{  wrapper: "min-h-[222px]", }}>
                        ...
                        <TableBody emptyContent={"No existen tipos"}>
                            {
                                sortedItems.map((tipo)=>(
                                    <TableRow key={tipo.id} className='text-start'>
                                        <TableCell className='overflow-hidden whitespace-nowrap text-ellipsis'>{tipo.id}</TableCell>
                                        <TableCell className='overflow-auto whitespace-nowrap text-ellipsis'>{tipo.nombre}</TableCell>
                                        <TableCell className='overflow-hidden whitespace-nowrap text-ellipsis'>
                                            <Tooltip content={"Editar"} color='warning'>
                                            <Link href={route('tipo-documento.edit',String(tipo.id))} >
...
export default ShowTiposDocumento
